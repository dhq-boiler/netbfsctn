...
internal static class ReportGenerator
{
    internal static void PrintConsole(AnalysisResult baseline, List<BenchmarkResult> results)
    {
        Console.WriteLine();
        Console.WriteLine("╔══════════════════════╦══════════╦═════════╦══════════╦═════════╦═══════════╦═══════════╦════════════╦══════════╗");
        Console.WriteLine("║ Scenario             ║ Size     ║ Size Δ% ║ Time(ms) ║ Time Δ% ║ Names R/T ║ Strings   ║ AvgIL/Meth ║ Correct? ║");
        Console.WriteLine("╠══════════════════════╬══════════╬═════════╬══════════╬═════════╬═══════════╬═══════════╬════════════╬══════════╣");

        PrintRow("Baseline", baseline.FileSizeBytes, 0, results[0].AvgRuntimeMs, 0,
        ...
        foreach (var r in results.Skip(1))
        {
            ...
            {
                PrintFailedRow(r.ScenarioName, r.ObfuscationError ?? "Unknown error");
                continue;
            }
            ...
            PrintRow(r.ScenarioName, a.FileSizeBytes, r.SizeDeltaPercent, r.AvgRuntimeMs, r.RuntimeDeltaPercent,
    ...
    private static void PrintRow(string name, long size, double sizeDelta, double timeMs, double timeDelta,
        string names, string strings, double avgIL, bool correct)
    {
        Console.WriteLine($"║ {name,-20} ║ {FormatSize(size),8} ║ {FormatDelta(sizeDelta),7} ║ {timeMs,8:F1} ║ {FormatDelta(timeDelta),7} ║ {names,9} ║ {strings,9} ║ {avgIL,10:F1} ║ {(correct ? "OK" : "FAIL"),8} ║");
    }

    private static void PrintFailedRow(string name, string error)
    {
        var truncated = error.Length > 70 ? error[..70] + "..." : error;
        Console.WriteLine($"║ {name,-20} ║ {"FAILED",8} ║ {"---",7} ║ {"---",8} ║ {"---",7} ║ {"---",9} ║ {"---",9} ║ {"---",10} ║ {"---",8} ║");
    }
    ...
    internal static void WriteMarkdown(string path, AnalysisResult baseline, List<BenchmarkResult> results)
    {
        ...
        sw.WriteLine();
        sw.WriteLine("| Scenario | Size | Size Δ% | Time(ms) | Time Δ% | Readable Names | Plaintext Strings | Avg IL/Method | Correct |");
        sw.WriteLine("|----------|------|---------|----------|---------|----------------|-------------------|---------------|---------|");

        WriteMarkdownRow(sw, "Baseline", baseline.FileSizeBytes, 0, results[0].AvgRuntimeMs, 0, baseline, true);
        ...
        foreach (var r in results.Skip(1))
        {
            ...
            WriteMarkdownRow(sw, r.ScenarioName, r.Analysis!.FileSizeBytes, r.SizeDeltaPercent,
                r.AvgRuntimeMs, r.RuntimeDeltaPercent, r.Analysis, r.CorrectnessPass);
        }
        ...
        foreach (var r in results.Skip(1).Where(r => r.ObfuscationSuccess))
        {
            ...
            sw.WriteLine($"- Runtime: {r.AvgRuntimeMs:F1}ms ({r.RuntimeDeltaPercent:+0.0;-0.0}%)");
    ...
    private static void WriteMarkdownRow(StreamWriter sw, string name, long size, double sizeDelta,
        double timeMs, double timeDelta, AnalysisResult a, bool correct)
    {
        ...
        sw.WriteLine($"| {name} | {FormatSize(size)} | {FormatDelta(sizeDelta)} | {timeMs:F1} | {FormatDelta(timeDelta)} | {readable}/{total} | {a.PlaintextStringCount} | {a.AvgILInstructionsPerMethod:F1} | {(correct ? "OK" : "FAIL")} |");
    }
}
